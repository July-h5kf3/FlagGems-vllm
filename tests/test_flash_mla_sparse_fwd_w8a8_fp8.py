...
from tests.flash_mla_fp8_utils import sparse_reference as dequantized_reference
...
def test_sparse_fp8_unified_small_shapes(batch, heads, topk):
    inputs, _, _ = make_inputs(batch, heads, topk, seed=123, magnitude=1.0)
    output, lse = flaggems_vllm.flash_mla_sparse_fwd_w8a8_fp8(*inputs)
    reference, reference_lse = dequantized_reference(inputs)
    assert_accuracy(output, lse, reference, reference_lse)
...
def test_fp8_mla_canonical_exports():
    ...
    assert (
        flaggems_vllm.flash_mla_with_kvcache_fwd_w8a8_fp8
        is flash_mla_with_kvcache_fwd_w8a8_fp8
    )
    assert "flash_mla_with_kvcache_fwd_w8a8_fp8" in ops.__all__
    assert "flash_mla_sparse_fwd_w8a8_fp8" in ops.__all__
    assert "flash_mla_ckv_fp8_per_token" not in ops.__all__
