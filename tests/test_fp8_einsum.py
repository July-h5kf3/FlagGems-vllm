...
def test_accuracy_fp8_einsum(shape):
    ...
    out = flaggems_vllm.fp8_einsum("bhr,hdr->bhd", x, xs, y, ys)
    b, h, r, d = shape
...
def test_einsum_precision_route(shape, dtype):
    ...
    out = flaggems_vllm.fp8_einsum("bhr,hdr->bhd", x, None, y, None, output_dtype=dtype)
...
def test_fp8_einsum_validation_and_extremes():
    ...
    with pytest.raises(ValueError, match="equation|supports"):
        flaggems_vllm.fp8_einsum("bij,bjk->bik", x, xs, y, ys)
    with pytest.raises(ValueError, match="scale"):
        flaggems_vllm.fp8_einsum("bhr,hdr->bhd", x, None, y, ys)
    with pytest.raises(TypeError, match="matching"):
        flaggems_vllm.fp8_einsum("bhr,hdr->bhd", x, xs, y.float(), ys)
...
def test_w8a8_block_fp8_bmm_interface(shape, dtype, provide_output):
    ...
    result = flaggems_vllm.w8a8_block_fp8_bmm(
...
def test_w8a8_block_fp8_bmm_output_validation():
    ...
    with pytest.raises(ValueError, match="shape"):
        flaggems_vllm.w8a8_block_fp8_bmm(x, y, xs.transpose(1, 2), ys)
    z = torch.empty((2, 3, 128), dtype=torch.float32, device=x.device)
    with pytest.raises(ValueError, match="dtype"):
        flaggems_vllm.w8a8_block_fp8_bmm(x, y, xs, ys, z=z)
    with pytest.raises(ValueError, match="scale"):
        flaggems_vllm.w8a8_block_fp8_bmm(x.bfloat16(), y.bfloat16(), xs, ys)
