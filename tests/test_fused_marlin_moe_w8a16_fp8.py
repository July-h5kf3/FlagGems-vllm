...
def utils():
    module = importlib.import_module("benchmark.test_fused_marlin_moe_w8a16_fp8")
...
def test_public_registration_fp8():
    assert flaggems_vllm.fused_marlin_moe_w8a16_fp8.__module__.startswith(
        "flaggems_vllm.runtime.backend._ascend"
    )
    assert "fused_marlin_moe_w8a16_fp8" in flaggems_vllm.ops.__all__


@pytest.mark.parametrize("m", [0, 1, 2, 3, 7, 8, 17, 32, 33, 40, 257])
@pytest.mark.parametrize("id_dtype", [torch.int32, torch.int64])
def test_accuracy_fp8(utils, ww, m, id_dtype):
...
def test_routing_edges_fp8(utils, ww, mode):
...
def test_non_power_of_two_geometry_fp8(utils, m, k, n):
...
def test_composite_max_activation_width_fp8(utils):
...
def test_sparse_and_dense_packing_fp8(utils, m):
...
def test_extended_geometry_fp8(utils, e, k, n, m):
