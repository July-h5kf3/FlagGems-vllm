...
PPU_FP8_MODEL_GEOMETRY = (256, 4096, 256, 6)
PPU_FP8_PR5140_TRACE = (
    (1, 172),
    (2, 172),
    (4, 172),
    (8, 172),
...
class FusedMarlinMoEW8A16FP8Benchmark(base.Benchmark):
    """Use the production trace from FlagGems PR 5140 on T-Head PPU."""

    def set_shapes(self, shape_file_path=None):
        num_experts, hidden_size, intermediate_size, top_k = PPU_FP8_MODEL_GEOMETRY
        self.shapes = [
            (m, num_experts, hidden_size, intermediate_size, top_k, count)
            for m, count in PPU_FP8_PR5140_TRACE
        ]
    ...
    def get_input_iter(self, dtype):
        ...
        for num_tokens, call_count in PPU_FP8_PR5140_TRACE:
            ...
            actual = _gems_call_fp8(*inputs)
            expected = _vllm_baseline_fp8(*inputs)
...
@pytest.mark.skipif(
    ...
    reason="vLLM FP8 fused_marlin_moe is not installed",
)
